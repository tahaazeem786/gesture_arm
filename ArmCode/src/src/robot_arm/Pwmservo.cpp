...
typedef struct {
  uint8_t pin_id;          /**< ESP32 GPIO pin ID for the servo. */
  int current_pulsewidth;  /**< Current pulse width of the servo (microseconds). */
  int actual_pulsewidth;   /**< Actual pulse width being output (current pulse width + offset). */
  bool pulsewidth_changed; /**< True if the target pulse width has been updated and motion is required. */
  bool is_running;         /**< True if the servo is currently moving to its target. */
  int target_pulsewidth;   /**< Target pulse width of the servo (microseconds). */
  int offset;              /**< Offset value applied to the servo pulse width (microseconds). */
  float pulsewidth_inc;    /**< Pulse width increment per tick. */
  uint32_t duration;       /**< Duration of the servo movement in milliseconds. */
  int inc_num;             /**< Remaining increment count for the movement. */
